...
collect_system_metrics() {
    ...
    # Disk metrics
    local ssd_total=$(df -B1 ${STORAGE_HDD_PATH:-/mnt/hdd} | awk 'NR==2 {print $2}')
    local ssd_used=$(df -B1 ${STORAGE_HDD_PATH:-/mnt/hdd} | awk 'NR==2 {print $3}')
    local ssd_available=$(df -B1 ${STORAGE_HDD_PATH:-/mnt/hdd} | awk 'NR==2 {print $4}')
    local ssd_usage_percent=$(df ${STORAGE_HDD_PATH:-/mnt/hdd} | awk 'NR==2 {print $5}' | sed 's/%//')
